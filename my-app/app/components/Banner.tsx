...
import { ArrowRight, Star, Truck, RotateCcw, Sparkles, Zap } from "lucide-react";
...
export function Banner() {
  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto grid max-w-[1480px] grid-cols-1 gap-12 px-6 lg:grid-cols-[1.05fr_1fr] lg:gap-16 lg:px-10 xl:gap-24">
        <div className="flex min-h-[60vh] flex-col justify-center lg:min-h-[78vh]">
          ...
          <p className="mt-8 max-full text-base text-justify leading-relaxed text-muted-foreground lg:text-lg">
            Minimal forms. Premium materials. A 24-piece capsule built for modern everyday living - sun, salt, motion and the long way home.
          </p>
        ...
        <div className="relative min-h-[60vh] lg:min-h-[78vh]">
          <div className="relative h-full w-full">
            <div className="absolute right-0 top-0 hidden h-[58%] w-[58%] overflow-hidden rounded-2xl shadow-xl md:block">
              <Image
                src="/hero.jpg"
            ...
            <div className="absolute inset-0 top-auto bottom-0 h-[78%] w-full overflow-hidden rounded-3xl bg-gradient-to-b from-[#e8ddd0] to-[#d4c4b0] shadow-2xl md:left-0 md:right-auto md:h-[72%] md:w-[72%]">
              <Image
                src="/hero.jpg"
            ...
            <div className="group absolute -left-2 bottom-8 z-10 w-[200px] cursor-default rounded-2xl border border-foreground/10 bg-background/90 p-4 shadow-xl backdrop-blur-md transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl md:left-6 md:bottom-12">
              <div className="flex items-center gap-1 text-accent">
                {STARS.map((_, i) => (
                  <Star key={i} className="h-3.5 w-3.5 fill-current" strokeWidth={0} />
                ))}
              </div>
              <div className="mt-2 font-serif text-2xl font-semibold leading-none text-foreground">4.9</div>
              <div className="mt-1 text-[11px] text-muted-foreground">From 2,847 verified reviews</div>
            </div>
            <div className="group absolute right-2 top-1/2 z-10 hidden -translate-y-1/2 rounded-2xl border border-foreground/10 bg-background/90 p-4 shadow-xl backdrop-blur-md transition-all duration-300 hover:scale-105 md:block">
              <div className="text-[10px] uppercase tracking-[0.3em] text-muted-foreground">Featured in</div>
              <div className="mt-1 font-serif text-lg font-semibold italic text-foreground">Monocle</div>
              <div className="text-[11px] text-muted-foreground">"Quietly excellent."</div>
            </div>
            <div className="absolute right-4 bottom-6 z-10 rounded-full border border-foreground/10 bg-foreground px-4 py-2 text-[11px] font-medium uppercase tracking-[0.2em] text-background shadow-xl">
              From $148
            </div>
          </div>
        </div>
